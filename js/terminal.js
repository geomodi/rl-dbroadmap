...
function setupTerminal() {
    ...
    function openTerminal() {
        ...
        anime({
            targets: terminal,
            height: 240, // Increased height to match CSS
            duration: 500,
            easing: 'easeOutQuad',
            complete: function() {
    ...
    function processCommand(command) {
        // Process command without showing the prompt
    ...
    function createEmailForm(prefilledMessage = '') {
        // Clear the terminal output first
        clearTerminal();
        ...
        form.innerHTML = `
            ...
            <div class="email-actions">
                <button id="send-email" class="send-button">SEND</button>
    ...
    // Function to send email with simplified animation
    function sendEmail() {
        ...
        // Clear terminal and show sending animation
        clearTerminal();
        addTerminalLine(`<span class="terminal-prompt">SYSTEM:</span> <span class="terminal-loading">Sending message...</span>`);
        scrollToBottom();
        ...
        emailjs.send('service_oo6j53p', 'template_s2qup8q', templateParams)
            .then(function(response) {
                ...
                setTimeout(() => {
                    clearTerminal();
                    addTerminalLine(`<span class="terminal-prompt">SYSTEM:</span> <span class="terminal-success">Message sent successfully!</span>`);
                ...
                setTimeout(() => {
                    clearTerminal();
                    addTerminalLine(`<span class="terminal-prompt">SYSTEM:</span> <span class="terminal-error">Message could not be sent.</span>`);
                    addTerminalLine(`<span class="terminal-prompt">SYSTEM:</span> <span class="terminal-error">Please check your connection and try again.</span>`);

                    // Add a button to try again
                    const tryAgainBtn = document.createElement('button');
                    tryAgainBtn.className = 'send-button';
                    ...
                    terminalOutput.appendChild(btnContainer);
    ...
    function startEmailMode() {
        ...
        // Start animation sequence - just one line for a cleaner interface
        addTerminalLine(`<span class="terminal-prompt">SYSTEM:</span> <span class="terminal-loading">Initializing messaging system...</span>`);
